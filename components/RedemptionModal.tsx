...
const RedemptionModal: React.FC<RedemptionModalProps> = ({ reward, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div 
        className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm text-center transform transition-all"
        onClick={(e) => e.stopPropagation()}
      >
        <img src={reward.imageUrl} alt={reward.name} className="w-24 h-24 rounded-lg mx-auto mb-4" />
        <h2 className="text-xl font-bold text-[#263238]">Confirmar Canje</h2>
        ...
        <div className="mt-6 flex gap-3">
          <button 
            onClick={onClose}
            className="w-full bg-gray-200 text-[#263238] font-semibold py-3 rounded-lg hover:bg-gray-300 transition-colors">
            Cancelar
          </button>
          <button 
